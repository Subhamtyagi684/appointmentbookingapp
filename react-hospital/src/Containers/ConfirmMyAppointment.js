...
class Contact extends Component{
    ...
    render(){
        return(
            <Fragment>
                <Header/>
                <center><h1 className='mt-2 mb-3'>Confirm Your Appointment Here!</h1></center>
                <section>
    ...
    componentDidMount(){
        let booking_id=this.props.match.params.booking_id
        if(booking_id){
            fetch(`http://127.0.0.1:8000/patientappointmentdetail/${booking_id}/`)
            .then((response)=> {
                if(response.status==200){
                    response.json()
                    .then((data)=> {
